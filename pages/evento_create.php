...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ((float)$anticipo_pagado > (float)$presupuesto_total) {
    $errors[] = 'El anticipo pagado no puede ser mayor que el presupuesto total.';
  }
  ...
  if (empty($errors)) {
    try {
      ...
      $sql = "INSERT INTO eventos (
        ...
        ubicacion, direccion_completa, numero_invitados,
            ...
            <div class="wizard-content">
                ...
                <div class="wizard-section" data-step="2">
                    ...
                    <div class="mb-3">
                        ...
                        <small class="text-muted">Esta dirección se usará para mostrar el mapa del evento</small>
                    </div>
                </div>
                ...
                <div class="wizard-section" data-step="4">
                    ...
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="number" step="0.01" class="form-control" name="presupuesto_total" id="presupuesto_total" min="0" placeholder="0.00" value="<?php echo esc($_POST['presupuesto_total'] ?? ''); ?>">
                        </div>
                        <div class="col-md-4 mb-3">
                            <label class="form-label"><i class="bi bi-check-circle me-1"></i>Anticipo Pagado</label>
                            <input type="number" step="0.01" class="form-control" name="anticipo_pagado" id="anticipo_pagado" min="0" placeholder="0.00" value="<?php echo esc($_POST['anticipo_pagado'] ?? ''); ?>">
                        </div>
                        <div class="col-md-4 mb-3">
                            <label class="form-label"><i class="bi bi-hourglass-split me-1"></i>Saldo Pendiente</label>
                            <input type="number" step="0.01" class="form-control bg-light" name="saldo_pendiente" id="saldo_pendiente" min="0" placeholder="0.00" value="<?php echo esc($_POST['saldo_pendiente'] ?? ''); ?>" readonly>
...
function validateCurrentStep() {
    const currentSection = document.querySelector(`.wizard-section[data-step="${currentStep}"]`);
...
function showValidationToast() {
    ...
    if (!toast) {
                ...
                <div id="validationToast" class="toast align-items-center text-white bg-danger border-0" role="alert" aria-live="assertive" aria-atomic="true">
                    <div class="d-flex">
                        <div class="toast-body">
                            <i class="bi bi-exclamation-circle me-2"></i>Por favor completa todos los campos obligatorios (marcados con *)
                        </div>
        ...
        toast = document.getElementById('validationToast');
    }
...
function calcularSaldo() {
    const presupuestoTotal = parseFloat(document.getElementById('presupuesto_total').value) || 0;
    let anticipoPagado = parseFloat(document.getElementById('anticipo_pagado').value) || 0;
    const anticipoField = document.getElementById('anticipo_pagado');
    
    // Validar que el anticipo no sea mayor que el presupuesto
    if (anticipoPagado > presupuestoTotal) {
        anticipoField.classList.add('is-invalid');
        
        // Mostrar mensaje de error si no existe
        let errorDiv = anticipoField.parentElement.querySelector('.invalid-feedback');
        if (!errorDiv) {
            errorDiv = document.createElement('div');
            errorDiv.className = 'invalid-feedback';
            anticipoField.parentElement.appendChild(errorDiv);
        }
        errorDiv.textContent = 'El anticipo no puede ser mayor que el presupuesto total';
        
        // Ajustar el anticipo al máximo permitido
        anticipoPagado = presupuestoTotal;
        anticipoField.value = presupuestoTotal.toFixed(2);
    } else {
        anticipoField.classList.remove('is-invalid');
        const errorDiv = anticipoField.parentElement.querySelector('.invalid-feedback');
        if (errorDiv) {
            errorDiv.remove();
        }
    }
    
    const saldoPendiente = presupuestoTotal - anticipoPagado;
    document.getElementById('saldo_pendiente').value = Math.max(0, saldoPendiente).toFixed(2);
}
